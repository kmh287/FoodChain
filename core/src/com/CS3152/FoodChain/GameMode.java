...
public class GameMode implements Screen {
  ...
  private static enum gameCondition {
    IN_PLAY, WIN, LOSE
  };
   ...
   *
   * The asset manager for LibGDX is asynchronous.That means that you tell it
   * what to load and then wait while it loads them.This is the first step:
   * telling it what to load.
   * 
   * @param manager
   ...
   *
   * The asset manager for LibGDX is asynchronous.That means that you tell it
   * what to load and then wait while it loads them.This is the second step:
  ...
  /**
   * Temporary constructor for GameMode until we have our architecture more
   * hammered-down. For now The game has no load state, goes immediately into
   * the game
   * 
   * @param canvas
   *          The singular instance of GameCanvas
   */
  ...
  private void initializeLevel(GameCanvas canvas, String levelName) {
    ...
    /*
     * size of animal list + the player controls = new
     * InputController[animals.size() + 1]; controls[0] = new
     * PlayerController(); tmp = new Vector2();
     */
    ...
    List<Actor.actorType> aTypes = map.getActorTypeList();
    ...
    trapController = new TrapController(hunter, map, collisionController,
        numPigs, numWolves);
    ...
    for (int i = 0; i < animals.size(); i++) {
      ...
      controls[i + 1] = new AIController(animals.get(i),
          collisionController.getWorld(), map, actors);
    }

    createSteeringBehaviors();

    // controls[1] = new AIController(animals.get(0),
    // collisionController.getWorld(),
    // map, actors);
    // controls[2] = new AIController(animals.get(1),
    // collisionController.getWorld(),
    // map, actors);
  ...
  /**
   * Function to wrap the try/catch required for loading a map in.
   * 
  ...
  /**
   * This function builds the animal list and is responsible for calling the
   * load functions for all necessary textures.
   * 
   * The same index i for both input lists refer to the same animal!
   * 
   * @param aTypes
   *          The list of animal types
   * @param coordinates
   *          the coordinates of the animals.
   */
  ...
  private void update(float delta) {
    ...
    // Check the objective every second, end the game if the player has won or
    // if the objective
    // cannot be achieved
    if (ticks % 60 == 0) {
    ...
    if (trapController.getSelectedTrap() == null
        || !trapController.canSetTrap()) {
  ...
  private void draw(float delta) {
    // canvas.begin();
    canvas.DrawBlack(GameMap.metersToPixels(hunter.getPosition().x),
        GameMap.metersToPixels(hunter.getPosition().y));
    canvas.end();
    ...
    // canvas.begin();
    if (!start) {
      canvas.beginCam(GameMap.metersToPixels(hunter.getPosition().x),
          GameMap.metersToPixels(hunter.getPosition().y));
    } else {
      canvas.beginCamStart(GameMap.metersToPixels(hunter.getPosition().x),
          GameMap.metersToPixels(hunter.getPosition().y));
      start = false;
    }
    ...
    /*
     * canvas.beginDebug(); for (Animal animal : animals) {
     * animal.drawSight(canvas); } canvas.endDebug();
     */
   ...
   *
   * Movement actions are determined, but not committed (e.g. the velocity is
   * updated, but not the position). Collisions are not processed.
   */
